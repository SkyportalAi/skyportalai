...
def test_token_from_env(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "spa-tok")
    cfg = AgentConfig.from_env()
    assert cfg.token == "spa-tok"


def test_missing_token_raises(monkeypatch):
    monkeypatch.delenv("SKYPORTALAI_AGENT_TOKEN", raising=False)
...
def test_whitespace_token_raises(monkeypatch):
    ...
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "   ")
    with pytest.raises(SkyportalError):
        AgentConfig.from_env()
...
def test_default_base_url(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
...
def test_base_url_from_env_trailing_slash_stripped(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_BASE_URL", "http://localhost:8000/")
    cfg = AgentConfig.from_env()
...
def test_defaults_when_unset(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    for var in (
        "SKYPORTALAI_AGENT_WANDB_DIR",
        "SKYPORTALAI_AGENT_MLFLOW_DIR",
        "SKYPORTALAI_AGENT_INTERVAL_SECONDS",
        "SKYPORTALAI_AGENT_ENABLE_WANDB",
        "SKYPORTALAI_AGENT_ENABLE_MLFLOW",
        "SKYPORTALAI_AGENT_CLUSTER_NAME",
    ):
...
def test_scan_dirs_interval_and_cluster_from_env(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_WANDB_DIR", "/data/wandb")
    monkeypatch.setenv("SKYPORTALAI_AGENT_MLFLOW_DIR", "/data/mlruns")
    monkeypatch.setenv("SKYPORTALAI_AGENT_INTERVAL_SECONDS", "30")
    monkeypatch.setenv("SKYPORTALAI_AGENT_CLUSTER_NAME", "gpu-cluster-1")
    cfg = AgentConfig.from_env()
...
def test_enable_toggle_parses_bool(monkeypatch, value, expected):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_ENABLE_WANDB", value)
    cfg = AgentConfig.from_env()
...
def test_invalid_interval_falls_back_to_default(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_INTERVAL_SECONDS", "not-an-int")
    cfg = AgentConfig.from_env()
    assert cfg.interval_seconds == 60
...
def test_non_positive_interval_falls_back_to_default(monkeypatch, value):
    ...
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_INTERVAL_SECONDS", value)
    cfg = AgentConfig.from_env()
    assert cfg.interval_seconds == 60
...
def test_from_env_accepts_explicit_mapping():
    cfg = AgentConfig.from_env(
        {"SKYPORTALAI_AGENT_TOKEN": "t", "SKYPORTALAI_AGENT_INTERVAL_SECONDS": "15"}
    )
    assert cfg.token == "t"
...
def test_state_dir_default_and_derived_paths(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.delenv("SKYPORTALAI_AGENT_STATE_DIR", raising=False)
    cfg = AgentConfig.from_env()
...
def test_state_dir_from_env(monkeypatch, tmp_path):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_STATE_DIR", str(tmp_path))
    cfg = AgentConfig.from_env()
...
def test_healthz_port_default_and_override(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.delenv("SKYPORTALAI_AGENT_HEALTHZ_PORT", raising=False)
    assert AgentConfig.from_env().healthz_port == 8080
    monkeypatch.setenv("SKYPORTALAI_AGENT_HEALTHZ_PORT", "9000")
...
def test_out_of_range_healthz_port_falls_back_to_default(monkeypatch, value):
    ...
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_HEALTHZ_PORT", value)
    assert AgentConfig.from_env().healthz_port == 8080


def test_queue_max_batches_default_and_override(monkeypatch):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.delenv("SKYPORTALAI_AGENT_QUEUE_MAX_BATCHES", raising=False)
    assert AgentConfig.from_env().queue_max_batches == 1000
    monkeypatch.setenv("SKYPORTALAI_AGENT_QUEUE_MAX_BATCHES", "50")
...
def test_invalid_queue_max_batches_falls_back_to_default(monkeypatch, value):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_QUEUE_MAX_BATCHES", value)
    assert AgentConfig.from_env().queue_max_batches == 1000
...
def test_invalid_int_logs_warning_naming_the_var(monkeypatch, caplog):
    ...
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_HEALTHZ_PORT", "8O80")  # letter O, not zero
    with caplog.at_level(logging.WARNING):
        cfg = AgentConfig.from_env()
    assert cfg.healthz_port == 8080
    assert "SKYPORTALAI_AGENT_HEALTHZ_PORT" in caplog.text


def test_out_of_range_int_logs_warning(monkeypatch, caplog):
    monkeypatch.setenv("SKYPORTALAI_AGENT_TOKEN", "t")
    monkeypatch.setenv("SKYPORTALAI_AGENT_HEALTHZ_PORT", "70000")  # > 65535
    with caplog.at_level(logging.WARNING):
        AgentConfig.from_env()
    assert "SKYPORTALAI_AGENT_HEALTHZ_PORT" in caplog.text
